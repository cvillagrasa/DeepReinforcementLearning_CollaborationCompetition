...
class MultiAgent(CriticHelper):
    ...
    def __post_init__(self):
        agent_params = {'state_size': self.state_size, 'action_size': self.action_size,
                        ...
                        'update_every': self.update_every, 'twin_delayed': self.twin_delayed, 'beta': self.beta,
                        'device': self.device}
        if self.share_critic:
            super().__post_init__()
            critic_common_data = self.setup_critic()
            agent_params['critic_common_data'] = critic_common_data
        if self.share_memory_replay:
            ...
            agent_params['memory_replay'] = self.memory_replay
...
class AgentDDPG(CriticHelper):
    ...
    lr_actor: float = 1e-4  # learning rate for the actor  3e-4
    update_every: ... = 1  # how often to update the network
    prioritized_experience_replay: ... = 0.  # 0 -> pure random sampling, 1 -> full prioritized experience replay
    twin_delayed: bool = False  # whether to implement the TD3 algorithm, instead of vanilla DDPG
    delayed_policy_factor: int = 2  # amount of times the Q network is more often updated than the policy
    e: float = 1e-8  # constant to prevent samples from being starved in prioritized experience replay
    beta: float = 0.5  # non-uniform probabilities compensation in importance sampling for prioritized experience replay
    device: ... = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
    warmup: float = 2000  # warmup timesteps taking random actions
    within_warmup: bool = True
    noise_exploration: float = 1.  # noise on selected actions
    noise_exploration_decay: float = 0.9999  # noise decay per step
    noise_exploration_minimum: float = 0.25
    within_noise_exploration_decay: bool = True
    ...
    def __post_init__(self):
        ...
        self.t_step = 0
        self.within_warmup = True if self.warmup > 0 else False
    ...
    def action(self, state, noise=True):
        if self.t_step < self.warmup:
            ...
            if self.within_warmup:
                print('\nWarmup period has ended')
                self.within_warmup = False
            ...
            if noise:
                ...
                self.noise_exploration *= self.noise_exploration_decay
                self.noise_exploration = np.max([self.noise_exploration, self.noise_exploration_minimum])
                if self.within_noise_exploration_decay and self.noise_exploration == self.noise_exploration_minimum:
                    print('\nExploration noise at its bottom')
                    self.within_noise_exploration_decay = False
    ...
    def learn(self, experiences, max_weight):
        ...
        if not self.twin_delayed or self.t_step % (self.update_every * self.delayed_policy_factor) == 0:
